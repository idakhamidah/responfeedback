        ...
        <h4><i class="fa fa-angle-right"></i> Probabilitas Prior</h4> 
            <table id="tabel2" class="display" cellspacing="0" width="100%" > 
                ...
                <tbody>
                  <?php
                    $no=0;
                    $probprior = mysql_query("select * from nbc_classpriors");
...
function prior ($Nc,$N){
  mysql_query("TRUNCATE nbc_classpriors");
  foreach ($Nc as $c => $c_val) {
    $prior[$c]=$Nc[$c]/$N;
    mysql_query("insert into nbc_classpriors values ('','$c','".number_format($prior[$c],14)."')");
  }
}
